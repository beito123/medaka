...
func NewLogger(out StdLogger) *Logger {
	return &Logger{
		Out:             out,
		OutLevel:        LevelDebug,
...
// Err logs the error
func (log *Logger) Err(err error, trace []*CallerInfo) {
	if trace == nil {
		trace = Dump(1, DefaultTraceLimit)
	}
...
// Trace logs traces
func (log *Logger) Trace(trace []*CallerInfo) {
	if trace == nil {
		trace = Dump(1, DefaultTraceLimit)
	}
